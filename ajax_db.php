...
function db_connect() {
    ...
    if ($_SERVER['SERVER_NAME']==='localhost0'){ //localhost0 used to force excecution of else on 'localhost'
        ...
        $result = new mysqli('162.241.24.68:3306', 'mystand6_freelancer', '!Freelancer1234!', 'mystand6_dog_weight_tracker');
        if (!$result) {
        throw new Exception('Could not connect to database server');
        } else {
        return $result;
...
function get_breed_weight_group($current_breed_name){
...
function get_breed_weight_group_auto($current_breed_name,$current_dog_gender,$weights,$dates,$pure_dob){
    require 'breed_info.php';

    foreach ($breed_info as $breed=>$info){  //if not auto
        if ($info[0] == $current_breed_name){ #$current_breed_name is acutally breed_id, but we use name to continue supporting early naming
...
function get_bounding_centiles_info($pure_dob,$last_weight,$last_weighting_date,$current_dog_gender,$weight_group){ //for standard weights
    ...
    try{
        ...
        if ($lower_bound_centile_id == -1){  // severly underweight, we cant predict
            return array();
        }

        if ($upper_bound_centile_id == 9){ //severly overwight, we cant predict
            return array();
        }
    ...
    catch(Exception $e) { //generic error catch. if there is error, return null, so that caller will know not to do prediction.
        return array();
    }
}
...
if(isset($_POST['action']) && !empty($_POST['action'])) {
    ...
    if ($_POST['action'] === "update_dog_weight"){
        $conn = db_connect();
        $id = intval($_POST['id']);
        $new_weight = floatval($_POST['new_weight']);
        $query= "UPDATE `dog_weight` SET `weight` = %f WHERE `dog_weight`.`id` = %d;";
        $query = sprintf($query,$new_weight,$id);
        $result = $conn->query($query);
    }
    if ($_POST['action'] === "delete_dog_weight"){
        $conn = db_connect();
        $id = intval($_POST['id']);
        $query= "DELETE FROM `dog_weight` WHERE `dog_weight`.`id` = %d;";
        $query = sprintf($query,$id);
        $result = $conn->query($query);

    }
    if ($_POST['action'] === "add_dog_weight"){
        $conn = db_connect();
        $new_weight = floatval($_POST['new_weight']);
        $dog_id_param = $_POST['dog_id_param'];

        $new_date = $_POST['new_date'];
        $query= "INSERT INTO `dog_weight` (`id`, `dog_id`, `weight`, `date_weighed`) VALUES (NULL, '%s', '%f', '%s');";
        $query = sprintf($query,$dog_id_param,$new_weight,$new_date);
        $result = $conn->query($query);

    }

    if ($_POST['action'] === "set_dog_id"){
        $dog_id = ($_POST['dog_id']);
        $_SESSION['dog_id'] = $dog_id;
        echo $_SESSION['dog_id'];
        
    }
    if ($_POST['action'] === "get_default_dog_id"){
        ...
        $result = $conn->query($query);
        $dog_id = $result->fetch_assoc();  #get the first one
        $dog_id  = $dog_id['dog_id'];
        echo $dog_id;

    }
    if ($_POST['action'] === "update_dog_gender"){
    
        $conn = db_connect();
        $id = intval($_POST['id']);
        $new_gender = $_POST['new_gender'];
        $query= "UPDATE `dog` SET `dog_gender` = %s WHERE `dog`.`dog_id` = %d;";
        $query = sprintf($query,$new_gender,$id);
        $result = $conn->query($query);
        echo ($query);
    }
    if ($_POST['action'] === "update_dog_weight_group"){
    
        $conn = db_connect();
        $id = intval($_POST['id']);
        $new_dog_weight_group = $_POST['new_weight_group'];
        $query= "UPDATE `dog` SET `dog_weight_group` = '%s' WHERE `dog`.`dog_id` = %d;";
        $query = sprintf($query,$new_dog_weight_group,$id);
        $result = $conn->query($query);
        echo ($query);
    }
    if ($_POST['action'] === "update_dog_breed"){
    
        $conn = db_connect();
        $id = intval($_POST['id']);
        $new_breed = $_POST['new_breed'];
        $query= "UPDATE `dog` SET `dog_breed` = %s WHERE `dog`.`dog_id` = %d;";
        $query = sprintf($query,$new_breed,$id);
        $result = $conn->query($query);
        echo ($query);
    }
    if ($_POST['action'] === "display_dog_weights"){
        $html = '';
        $conn = db_connect();

        $valid_user = $_POST['valid_user'];
        $dog_id_param = $_POST['dog_id_param'];
        $dog_weight_unit = $_POST['dog_weight_unit'];
        ...
        $query = "SELECT `dog_id`,`dog_name`,`dog_breed`,`dog_dob`,`dog_gender`,`dog_weight_group` FROM `dog` WHERE `user_id` = '%s';";
        $query = sprintf($query,$user_id);
        $result = $conn->query($query);
        ...
        $dog_weight_group = $row['dog_weight_group'];

        $dog_ids[] = $dog_id;
        $dog_names[] = $dog_name;
        $dog_breeds[] = $dog_breed;
        $dog_dobs[] = $dog_dob;
        $dog_genders[] = $dog_gender;
        $dog_weight_groups[] = $dog_weight_group;
        }

        $dog_ids_names = array_combine($dog_ids,$dog_names);
        $dog_ids_breeds = array_combine($dog_ids,$dog_breeds);
        $dog_ids_dobs = array_combine($dog_ids,$dog_dobs);
        $dog_ids_genders = array_combine($dog_ids,$dog_genders);
        $dog_ids_weight_groups = array_combine($dog_ids,$dog_weight_groups);


        if (isset($_SESSION['dog_id']) & $dog_id_param == '-1'){  #for form entry support, javascript version more elegant
            $dog_id_param = $_SESSION['dog_id'];
           
        }

        else if (!isset($_SESSION['dog_id']) & $dog_id_param == '-1'){

            $dog_id_param = $dog_ids[0];  //use the first one as default
            
        }

        $html = $html.'<label for="dog">Choose which of your dogs:</label>';
        $html = $html.'<select name="dog_name_id" id="dog_name_id" onchange="getadog()">';
        $temp_index = 0;
        foreach($dog_ids_names as $dog_id=> $dog_name){
            
            if ($dog_id == $dog_id_param){
                $html = $html.'<option value="'.$dog_id.'" selected>'.$dog_name.'</option>';
            }
            else{
                $html = $html.'<option  value="'.$dog_id.'">'.$dog_name.'</option>';
            }
        ...
        $dog_id = $dog_id_param;  //use the first one as default
        
        $current_dog_name = $dog_ids_names[$dog_id];
        $current_dog_name = strtoupper($current_dog_name);

        $current_dog_breed =  $dog_ids_breeds[$dog_id];
        $current_dog_dob =  $dog_ids_dobs[$dog_id];
        
        $current_dog_gender =  $dog_ids_genders[$dog_id];

        $current_dog_weight_group =  $dog_ids_weight_groups[$dog_id];



        $html = $html.'<label for="gender">Dog Gender:</label>';
        $html = $html.'<select name="dog_gender_id" id="dog_gender_id" onchange="updatedoggender()">';


        //$html = $html.'<option  value="'.$dog_id.'" selected>'.$current_dog_gender.'</option>';

        if ($current_dog_gender == "1"){
            $html = $html.'<option value= 1 selected>Female</option>';
            $html = $html.'<option value= 2>Male</option>';
        }
        if ($current_dog_gender == "2"){
            $html = $html.'<option value= 2 selected>Male</option>';
            $html = $html.'<option value= 1>Female</option>';
        ...
        $html = $html.'<label for="dog_breed_id">Dog Breed:</label>';
        $html = $html.'<select name="dog_breed_id" id="dog_breed_id" onchange="updatedogbreed()">';
        ...
        ksort($breed_info);
        foreach ($breed_info as $breed=>$info){
            if ($current_dog_breed == $info[0]){
                $html = $html.'<option value= '.$info[0].' selected>'.$breed.'</option>';  //select the breed in db
            }
            else{
                $html = $html.'<option value= '.$info[0].'>'.$breed.'</option>';
            }
        }
        ...
        //breed weight group
        $html = $html.'<label for="dog_breed_weight_group">Dog Breed Weight Group:</label>';
        $html = $html.'<select name="dog_breed_weight_group" id="dog_breed_weight_group" onchange="updatedogbreedweightgroup()">';


        //$html = $html.'<option  value="'.$dog_id.'" selected>'.$current_dog_gender.'</option>';
        $weight_groups = array(''=>'','Toy Breed'=>'i','Small Breed'=>'ii','Medium Breed'=>'iii','Large Breed'=>'iv','Giant Breed'=>'v','Automatic'=>'auto');
        foreach ($weight_groups as $key=>$weight_group){
            if ($current_dog_weight_group === $weight_group){
                $html = $html.'<option value= '.$weight_group.' selected>'.$key.'</option>';  //select the breed in db
            }
            else{
                $html = $html.'<option value= '.$weight_group.'>'.$key.'</option>';
            }
        }
        ...
        $html = $html.'<p><b>Weight Unit</b></p>';
        if ($dog_weight_unit == 'lbs'){
            $html = $html.'<input type="radio" id="lbs" name="weight_unit_option" onclick="weightunitchange(this);" checked value="lbs" />';
            $html = $html.'<label for="lbs">Pounds</label><br>';
            $html = $html.'<input type="radio" id="kg" name="weight_unit_option" onclick="weightunitchange(this);" value="kg" />';
            $html = $html.'<label for="kg">Kilograms</label><br>';
        }
        if ($dog_weight_unit == 'kg'){
            $html = $html.'<input type="radio" id="lbs" name="weight_unit_option" onclick="weightunitchange(this);"  value="lbs" />';
            $html = $html.'<label for="lbs">Pounds</label><br>';
            $html = $html.'<input type="radio" id="kg" name="weight_unit_option" onclick="weightunitchange(this);" checked value="kg" />';
            $html = $html.'<label for="kg">Kilograms</label><br>';
        }

        $html = $html.'<br>';
        $html = $html.'<br>';
        $query = "SELECT `id`,`date_weighed`,`weight` FROM `dog_weight` WHERE `dog_id`='%s' ORDER BY date_weighed ASC;";
        $query = sprintf($query,$dog_id);
        $result = $conn->query($query);

        $dates = array();
        $weights = array();
        //$html = $html.'<div id="dashboard-div">'; //start of dashboard div
        $html = $html.'<input type="text" readonly id="datetitle" name="lname" value="Date">&nbsp;';
        $html = $html.'<input type="text" id="weighttitle" name="lname" value="Weight">&nbsp;';
        $html = $html.'<br><br>';
        while($row = $result->fetch_assoc()) {
            $id = strval($row["id"]);
            $date = $row["date_weighed"];
            $weight = $row["weight"];

            $html = $html.'<input type="date" readonly id="date'.$id.'" name="date" value="'.$date.'">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
            if ($dog_weight_unit == 'lbs'){
                $html = $html.'<input type="text" readonly id="weight'.$id.'" name="weight" value="'.$weight.'">&nbsp;';
            }
            if ($dog_weight_unit == 'kg'){
                //data is saved to db in unit of lbs as default
                //user wants dispaly in kg, so we convert before display
                $KG_TO_POUND = 2.20462;
                $POUND_TO_KG = 1/$KG_TO_POUND;
                $weight_kg = strval((float)$weight*$POUND_TO_KG);
                $html = $html.'<input type="text" readonly id="weight'.$id.'" name="weight" value="'.$weight_kg.'">&nbsp;';
            }
            $html = $html.'<button type="button" id="editbtn'.$id.'" onclick="editClick('.$id.')" >Edit</button>&nbsp;';
            $html = $html.'<button type="button" id ="deletebtn'.$id.'" onclick="deleteClick('.$id.')">Delete</button>';
            $html = $html.'<br><br>';
        ...
        $html = $html.'<p style="color:green;font-size:1.5em;">Your dog`s <b><span style="color:blue;">current growth pattern</span></b> should closely follow ONE of the standard curves 1 to 9 below if your dog is growing normally.</p>';
        $html = $html.'<div id="graph_canvas" style="width:600px;height:500px;"></div>';
        //$html = $html.'</div>'; //end of dashboard div
        $html = $html.'<label>Add New Weight Record For '.$current_dog_name.'</label><br><br>';
        $html = $html.'<input type="date"  id="new_date" name="date" required>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
        $html = $html.'<input type="text"  id="new_weight" name="weight" required>&nbsp;';
        $html = $html.'<button type="button" id ="addbtn" onclick="addClick('.$dog_id_param.')">Save</button>';
        $html = $html.'<button type="button" id ="breedbtn" >'.$current_dog_breed.'</button>';
        $html = $html.'<button type="button" id ="dobbtn" >'.$current_dog_dob.'</button>';
        $html = $html.'<button type="button" id ="genderbtn" >hello'.$dog_id.'</button>';

        
        $pure_dob = $current_dog_dob;  # pure dob is dob string that has not been converted to datetime object. need this as a fix because current date somehow be
        $current_dog_dob=new DateTime($current_dog_dob);

        
        $todays_date = new DateTime(date("Y-m-d"));
        $min_recorded_date = $dates[0];

        $current_dog_conception_date = clone $current_dog_dob; //clone because sub operation that comes next line will mutate $current_dog_dob, we do not want that 
        
        $current_dog_conception_date->sub(new DateInterval('P63D')); #//dogs are pregnant for approximaely 63 days


        //$dates = array_merge(array($current_dog_conception_date->format('Y-m-d')),$dates);  //include date of conception with data
        //$weights = array_merge(array(strval(0)),$weights);  //NOTE weight at conceptoion is zero
        
        #echo "difference " . $interval->y . " years, " . $interval->m." months, ".$interval->d." days "; 
        
        $dog_age = $current_dog_dob->diff($todays_date);
        $current_dog_age_in_days = $dog_age->days;

        $html = $html.'<button type="button" id ="agebtn" >'.$current_dog_age_in_days.'</button>';
        $html = $html.'<button type="button" id ="mindatebtn" >'.$min_recorded_date.'</button>';

        //$weight_group = get_breed_weight_group($current_dog_breed);
        //$weight_group =  get_breed_weight_group_auto($current_breed_name,$current_dog_gender,$weights,$dates,$pure_dob);

        if (($current_dog_weight_group == '') | ($current_dog_weight_group == 'auto')){
            $weight_group =  get_breed_weight_group_auto($current_dog_breed,$current_dog_gender,$weights,$dates,$pure_dob);
        }
        else{
            $weight_group = $current_dog_weight_group;
        }
        ...
        echo json_encode(array('html'=> $html, 
            'weights' => $weights,'dates'=>$dates,
            'dog_breed'=> $current_dog_breed,
            'standard_dates'=> $standard_dates,
            'standard_centiles' => $standard_centiles,
            'projected_weights' =>$projected_weights,
            'projected_dates' => $projected_dates,
            'lower_bound_centiles' =>$lower_bound_centiles,
            'post' =>$_POST['action']
        ));
        }
